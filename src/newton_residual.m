function [p_res, d_pdelta, p_norms] = newton_residual(delta, XX1, YY1, XX2, YY2, m, n, pl, alpha10_c, alpha01_c, eps_)
...
for i = 1:((m + 1) * n)
    ...
    dpx = dp_val(px1, px2, alpha10_c(i), delta, eps_);
    [ddxx, ~] = grad_dp(px1, px2, 0, alpha10_c(i), delta, eps_);
    ...
    if (YY2(shift_i - (m + 1) * n) == 0)
        ...
        [~, ddxy] = grad_dp(px1, px2, pl(shift_i), alpha10_c(i), delta, eps_);
        v1(5*i - 3) = 1/4 * ddxy;
    end
    ...
    if (YY2(shift_i - 1 - (m + 1) * n) == 0)
        ...
        [~, ddxy] = grad_dp(px1, px2, pl(shift_i - 1), alpha10_c(i), delta, eps_);
        v1(5*i - 2) = 1/4 * ddxy;
    end
    ...
    if (YY2(shift_i + m - 1 - (m + 1) * n) == n)
        ...
        [~, ddxy] = grad_dp(px1, px2, pl(shift_i + m - 1), alpha10_c(i), delta, eps_);
        v1(5*i - 1) = 1/4 * ddxy;
    end
    ...
    if (YY2(shift_i + m - (m + 1) * n) == n)
        ...
        [~, ddxy] = grad_dp(px1, px2, pl(shift_i + m), alpha10_c(i), delta, eps_);
        v1(5*i) = 1/4 * ddxy;
    end
...
for i = 1:((n + 1) * m)
    ...
    dpy = dp_val(px2, px1, alpha01_c(i), delta, eps_);
    [ddyy, ~] = grad_dp(px2, px1, 0, alpha01_c(i), delta, eps_);
    ...
    if (XX1(i - m - 1 + YY2(i)) == 0)
        ...
        [~, ddxy] = grad_dp(px2, px1, pl(i - m - 1 + YY2(i)), alpha01_c(i), delta, eps_);
    ...
    if XX1(i - m - 1 + YY2(i) + 1) == m
        ...
        [~, ddxy] = grad_dp(px2, px1, pl(i - m - 1 + YY2(i) + 1), alpha01_c(i), delta, eps_);
    ...
    if XX1(i + YY2(i)) == 0
        ...
        [~, ddxy] = grad_dp(px2, px1, pl(i + YY2(i)), alpha01_c(i), delta, eps_);
    ...
    if XX1(i + YY2(i) + 1) == m
        ...
        [~, ddxy] = grad_dp(px2, px1, pl(i + YY2(i) + 1), alpha01_c(i), delta, eps_);
...
function y = dp_val(p1, p2, alpha, delta, eps_)
    p_norm = sqrt(p1^2 + p2^2);
    if p_norm <= alpha
        y=0;
        return
    end
    y = p1 / (p_norm * eps_) * d_smooth_pen(p_norm, alpha, delta);
end
 
function [ddxx, ddxy] = grad_dp(p1, p2, p2i, alpha, delta, eps_)
    p_norm = sqrt(p1^2 + p2^2);
    if p_norm <= alpha
        ddxx = 0;
        ddxy = 0;
    ...
    ddxx = p2^2 / (p_norm^3 * eps_) * d_smooth_pen(p_norm, alpha, delta) + p1^2 / (p_norm^2 * eps_) * dd_smooth_pen(p_norm, alpha, delta);
    ddxy = -p1*p2 / (p_norm^3 * eps_) * d_smooth_pen(p_norm, alpha, delta) + p1 * p2 / (p_norm^2 * eps_) * dd_smooth_pen(p_norm, alpha, delta);
end
